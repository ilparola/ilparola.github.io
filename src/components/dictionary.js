...
const Dictionary = ({ onClose }) => {
  ...
  const [filterType, setFilterType] = useState("all"); // all, generated, captured
  const [selectedLetter, setSelectedLetter] = useState("");

  // Carica ed unisci le parole contrassegnandole con la sorgente
  const allWords = useMemo(() => {
    const gen = getGeneratedWords(true).map((w) => ({ word: w, type: "generated" }));
    const cap = getCapturedWords(true).map((w) => ({ word: w, type: "captured" }));
    
    // Unisci ed elimina doppioni logici
    const merged = [...gen];
    cap.forEach((cWord) => {
      const existing = merged.find((m) => m.word.toLowerCase() === cWord.word.toLowerCase());
      if (existing) {
        // Se esiste in entrambi, lo segniamo come 'both' o lo teniamo come captured
        existing.type = "both";
      } else {
        merged.push(cWord);
      }
  ...
  const filteredWords = useMemo(() => {
    return allWords.filter((item) => {
      ...
      else if (filterType === "generated") matchType = !isRaddoppio && (item.type === "generated" || item.type === "both");
      else if (filterType === "captured") matchType = !isRaddoppio && (item.type === "captured" || item.type === "both");
    ...
    <div className="animate-fade-in" style={{ display: "flex", flexDirection: "column", gap: "1.5rem", width: "100%" }}>
      ...
      <div className="glass-panel" style={{ padding: "1.25rem", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "1rem" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
          <button className="btn btn-dark btn-icon-only" onClick={onClose} title="Torna al gioco">
            <FaArrowLeft />
          </button>
          <div style={{ display: "flex", flexDirection: "column" }}>
            <h2 className="title" style={{ fontSize: "1.25rem", fontWeight: 800, textTransform: "uppercase", margin: 0, color: "var(--color-primary)" }}>
        ...
        <div className="words-count-badge" style={{ fontSize: "0.9rem", padding: "0.4rem 0.8rem" }}>
      ...
      <div className="glass-panel" style={{ padding: "1.5rem", display: "flex", flexDirection: "column", gap: "1rem" }}>
        <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: "1rem", flexWrap: "wrap" }}>
          ...
          <div style={{ position: "relative" }}>
            ...
            />
            <FaSearch style={{ position: "absolute", left: "14px", top: "50%", transform: "translateY(-50%)", color: "var(--color-text-muted)" }} />
          </div>
          ...
          <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
            ...
            <button
              className={`btn btn-dark ${filterType === "generated" ? "btn-primary" : ""}`}
        ...
        {/* Legenda ed info */}
        <div style={{ display: "flex", gap: "1rem", flexWrap: "wrap", fontSize: "0.78rem", color: "var(--color-text-muted)", borderTop: "1px solid rgba(255,255,255,0.05)", paddingTop: "0.75rem" }}>
          <div>🤖 <b style={{color: "var(--color-text)"}}>IA</b>: Parole generate artificialmente tramite Intelligenza Artificiale.</div>
          <div>📺 <b style={{color: "var(--color-accent)"}}>TV</b>: Parole reali tratte dalle puntate della trasmissione televisiva "Reazione a Catena".</div>
          <div>✨ <b style={{color: "var(--color-success)"}}>Entrambi</b>: Parole presenti in entrambi i dizionari.</div>
          <div>🔀 <b style={{color: "#c084fc"}}>Raddoppio</b>: Parola composta da più termini (es. "new york") &mdash; variante esclusiva del gioco.</div>
        </div>

        {/* Filtro Alfabetico A-Z */}
        <div style={{ borderTop: "1px solid rgba(255,255,255,0.05)", paddingTop: "1rem" }}>
          <div style={{ fontSize: "0.75rem", fontWeight: 700, textTransform: "uppercase", color: "var(--color-text-muted)", marginBottom: "0.5rem", display: "flex", justifyContent: "space-between" }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: "0.25rem" }}>
            {alphabet.map((letter) => (
              <button
                ...
                style={{ width: "32px", height: "32px", padding: 0, fontSize: "0.8rem", borderRadius: "4px" }}
