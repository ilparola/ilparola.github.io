...
const SummaryModal = ({
  ...
  // Statistiche
  const totalWords =
    guessedWords.length + errorWords.length + passedWords.length;
  const accuracy =
    totalWords > 0 ? ((guessedWords.length / totalWords) * 100).toFixed(0) : 0;

  const sumTime = (arr) => arr.reduce((acc, curr) => acc + (curr.time || 0), 0);
  const totalTimeSpent =
    sumTime(guessedWords) + sumTime(errorWords) + sumTime(passedWords);
  const avgTime =
    totalWords > 0 ? (totalTimeSpent / totalWords).toFixed(1) : "0.0";
  const avgCorrectTime =
    guessedWords.length > 0
      ? (sumTime(guessedWords) / guessedWords.length).toFixed(1)
      : "0.0";
    ...
    <div className={`modal ${isOpen ? "is-active" : ""}`}>
      ...
      <div className="modal-card" ref={componentRef}>
        ...
        <section className="modal-card-body">
          ...
          <div className="modal-stats-grid">
            <div className="stat-box">
              <div className="stat-box-label">
                <FaTrophy
                  style={{ color: "var(--color-accent)", marginRight: "4px" }}
                />{" "}
                Punti
              </div>
            ...
            <div className="stat-box">
              <div className="stat-box-label">
                <FaPercentage
                  style={{ color: "var(--color-success)", marginRight: "4px" }}
                />{" "}
                Precisione
              </div>
            ...
            <div className="stat-box">
              <div className="stat-box-label">
                <FaStopwatch
                  style={{ color: "var(--color-primary)", marginRight: "4px" }}
                />{" "}
                Tempo Medio
              </div>
          ...
          >
            <div>
              Parole Totali Giocate: <b>{totalWords}</b>
            </div>
            <div>
              Tempo Totale Risposta: <b>{totalTimeSpent.toFixed(1)}s</b>
            </div>
            <div>
              Media Generica Risposta: <b>{avgTime}s</b>
            </div>
            <div>
              Parole Non Indovinate:{" "}
              <b>{errorWords.length + passedWords.length}</b>
            </div>
          </div>
            ...
            {guessedWords.length > 0 && (
              <>
                <h3 className="modal-summary-section-title correct">
                  <FaCheck style={{ marginRight: "6px" }} /> Indovinate (
                  {guessedWords.length})
                </h3>
            ...
            {errorWords.length > 0 && (
              <>
                <h3 className="modal-summary-section-title errors">
                  <FaBan style={{ marginRight: "6px" }} /> Sbagliate (
                  {errorWords.length})
                </h3>
                <div className="modal-pill-wrap">
            ...
            {passedWords.length > 0 && (
              <>
                <h3 className="modal-summary-section-title passed">
                  <FaArrowRight style={{ marginRight: "6px" }} /> Passate (
                  {passedWords.length})
                </h3>
                <div className="modal-pill-wrap">
